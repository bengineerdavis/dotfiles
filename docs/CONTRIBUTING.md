...
> **This file is superseded.** The live conventions are in
> `dotfiles/docs/CONVENTIONS.md` — that is what the 39 topic files cite, and it is
> the one kept current. `dotfiles/docs/CONTRIBUTING.md` holds the workflow around
> it. Commit granularity in particular lives in CONVENTIONS.md § Commit
> granularity; do not restate it here.
>
> Kept for now because the rules below still read well as a summary, but edit the
> `dotfiles/docs/` copies instead.
